...
//referenciamos los bits de inicio y de fin del archivo html con ensamblador
extern const char html_bits_start[] asm("_binary_index_html_start");
//binary para acceder en binario, index_html el nombre del archivo y start para los bits de inicio
extern const char html_bits_end[] asm("_binary_index_html_end");

//hacemos lo mismo para el archivo de la rueda cromática.
extern const char chroma_bits_start[] asm("_binary_chroma_png_start");
extern const char chroma_bits_end[] asm("_binary_chroma_png_end");
...
static esp_err_t api_holamundo_generator(httpd_req_t *req){
    ...
    if(len_buf > 1){ //si hay un query en el url (len_buf > 1 ) entonces hacemos lo siguiente:
        ...
        if(httpd_req_get_url_query_str(req, buf, len_buf) == ESP_OK){; // tomamos el query del url en el req (respuesta) y lo metemos en buf, le pasamos ademas el tamaño
            char param[32];//un char para guardar 255 a 0 como str
...
//No te olvides de pasar el puntero de req como parámetro de la función
static esp_err_t home_get_handler(httpd_req_t *req){
    httpd_resp_set_type(req, "text/html");
    //para buscar el tamaño del archtivo html hacemos lo siguiente: 
    const uint32_t index_len = html_bits_end - html_bits_start;
    httpd_resp_send(req, html_bits_start, index_len);//le pasamos el inicio del archivo y el tamaño.
...
static esp_err_t chroma_get_handler(httpd_req_t *req){
    ...
    uint32_t image_len = chroma_bits_end - chroma_bits_start;
    httpd_resp_send(req, chroma_bits_start, image_len);
    return ESP_OK;
}
...
void http_init(){
    ...
    if (httpd_start(&server, &config) == ESP_OK){
        ...
        httpd_register_uri_handler(server, &home); //no olvides registrar los handler nuevos
